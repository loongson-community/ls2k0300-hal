  ...
  [..]
    ...
    *** DMA mode IO MEM operation ***
     ...
     *** I2C Workarounds linked to Silicon Limitation ***
     ====================================================
     [..]
       Below the list of all silicon limitations implemented for HAL on LS2K03xx product.
...
HAL_StatusTypeDef HAL_I2C_Init(I2C_HandleTypeDef *hi2c)
{
  ...
  /* Check the minimum allowed APB frequency */
  if (I2C_MIN_APB_FREQ(apb, hi2c->Init.ClockSpeed) == 1U)
  {
    return HAL_ERROR;
  }

  /* Calculate frequency range */
  freqrange = I2C_FREQRANGE(apb);
  ...
  MODIFY_REG(hi2c->Instance->CCR, (I2C_CCR_FS | I2C_CCR_DUTY | I2C_CCR_CCR), I2C_SPEED(apb, hi2c->Init.ClockSpeed, hi2c->Init.DutyCycle));
  ...
  if (hi2c->State == HAL_I2C_STATE_READY)
  {
    ...
    if (hi2c->XferSize > 0U)
    {
      if (hi2c->hdmatx != NULL)
      {
        ...
        dmaxferstatus = HAL_DMA_Start_IT(hi2c->hdmatx, (uint32_t)(uint64_t)hi2c->pBuffPtr, (uint32_t)(uint64_t)&hi2c->Instance->DR, hi2c->XferSize);
      }
      else
      {
        /* Update I2C state */
...
HAL_StatusTypeDef HAL_I2C_Master_Receive_DMA(I2C_HandleTypeDef *hi2c, uint16_t DevAddress, uint8_t *pData, uint16_t Size)
{
  ...
  /* Size <= 2: Use HAL_I2C_Master_Receive_IT */
  if (Size <= 2U)
  {
  ...
  if (hi2c->State == HAL_I2C_STATE_READY)
  {
    ...
    /* Size > 2: DMA transfer (Size-2) bytes, remaining 2 bytes will be handled in DMA callback */
    if (hi2c->hdmarx != NULL)
    {
      /* Set the I2C DMA transfer complete callback */
      ...
      /* Enable the DMA channel */
      /* DMA transfer Size-2 bytes, the last 2 bytes will be received in I2C_DMAXferCplt */
      dmaxferstatus = HAL_DMA_Start_IT(hi2c->hdmarx, (uint32_t)(uint64_t)&hi2c->Instance->DR, (uint32_t)(uint64_t)hi2c->pBuffPtr, hi2c->XferSize - 2U);
    }
    else
    {
      /* Update I2C state */
...
HAL_StatusTypeDef HAL_I2C_Slave_Receive_DMA(I2C_HandleTypeDef *hi2c, uint8_t *pData, uint16_t Size)
{
  ...
  if (hi2c->State == HAL_I2C_STATE_READY)
  {
    ...
    if (hi2c->hdmarx != NULL)
    {
      ...
      /* Enable the DMA channel */
      dmaxferstatus = HAL_DMA_Start_IT(hi2c->hdmarx, (uint32_t)(uint64_t)&hi2c->Instance->DR, (uint32_t)(uint64_t)hi2c->pBuffPtr, hi2c->XferSize);
    }
    else
    {
      /* Update I2C state */
...
HAL_StatusTypeDef HAL_I2C_Mem_Read_DMA(I2C_HandleTypeDef *hi2c, uint16_t DevAddress, uint16_t MemAddress, uint16_t MemAddSize, uint8_t *pData, uint16_t Size)
{
  ...
  /* Size <= 2: Use HAL_I2C_Mem_Read_IT */
  if (Size <= 2U)
  {
  ...
  if (hi2c->State == HAL_I2C_STATE_READY)
  {
    ...
    if (hi2c->hdmarx != NULL)
    {
      ...
      /* Enable the DMA channel */
      /* DMA transfer Size-2 bytes, the last byte will be received in I2C_DMAXferCplt */
      dmaxferstatus = HAL_DMA_Start_IT(hi2c->hdmarx, (uint32_t)(uint64_t)&hi2c->Instance->DR, (uint32_t)(uint64_t)hi2c->pBuffPtr, hi2c->XferSize - 2U);
    }
    else
    {
...
HAL_StatusTypeDef HAL_I2C_Master_Seq_Receive_DMA(I2C_HandleTypeDef *hi2c, uint16_t DevAddress, uint8_t *pData, uint16_t Size, uint32_t XferOptions)
{
  ...
  /* Size <= 2: Use HAL_I2C_Master_Seq_Receive_IT */
  if (Size <= 2U)
  {
    return HAL_I2C_Master_Seq_Receive_IT(hi2c, DevAddress, pData, Size, XferOptions);
  }
...
static void I2C_DMAXferCplt(DMA_HandleTypeDef *hdma)
{
  ...
  /* Master Receive DMA: Size > 2, DMA transferred Size-2 bytes, the last 2 bytes (Size-1th and Size-th) manually
     Note: Since hardware lacks CR2_LAST bit, we use POS + NACK + STOP to properly terminate transfer */
  else if ((CurrentState == HAL_I2C_STATE_BUSY_RX) && (CurrentMode == HAL_I2C_MODE_MEM) && (CurrentXferOptions == I2C_NO_OPTION_FRAME))
  {
    /* Disable DMA Request */
    CLEAR_BIT(hi2c->Instance->CR2, I2C_CR2_DMAEN);

    /* Read the byte already in DR (Size-1th byte, received before DMA TC) */
    *hi2c->pBuffPtr = (uint8_t)hi2c->Instance->DR;
    hi2c->pBuffPtr++;

    /* Update counters - 1 byte remains (the last one) */
    hi2c->XferCount = 1U;
    hi2c->XferSize = 1U;

    /* Enable Pos: needed for proper NACK of last byte */
    SET_BIT(hi2c->Instance->CR1, I2C_CR1_POS);

    /* Disable Acknowledge before receiving last byte */
    CLEAR_BIT(hi2c->Instance->CR1, I2C_CR1_ACK);

    /* Wait until RXNE flag is set (Size-th byte received, latched after reading Size-1th) */
    if (I2C_WaitOnRXNEFlagUntilTimeout(hi2c, I2C_TIMEOUT_FLAG, HAL_GetTick()) != HAL_OK)
    {
      /* Timeout or Error - read DR anyway to clear buffer */
      (void)hi2c->Instance->DR;
    }
    else
    {
      /* Read the last byte (Size-th) */
      *hi2c->pBuffPtr = (uint8_t)hi2c->Instance->DR;
      hi2c->pBuffPtr++;
    }
    ...
    if (I2C_WaitOnSTOPFlagUntilTimeout(hi2c, I2C_TIMEOUT_FLAG, HAL_GetTick()) != HAL_OK)
    {
      /* Timeout on STOP wait - set error and continue */
      hi2c->ErrorCode |= HAL_I2C_ERROR_TIMEOUT;
    }
  ...
  /* Master Receive DMA: Size > 2, DMA transferred Size-2 bytes, the last 2 bytes (Size-1th and Size-th) manually
     Note: Since hardware lacks CR2_LAST bit, we use POS + NACK + STOP to properly terminate transfer */
  else if ((CurrentState == HAL_I2C_STATE_BUSY_RX) && (CurrentMode == HAL_I2C_MODE_MASTER) && (CurrentXferOptions == I2C_NO_OPTION_FRAME))
  {
    /* Disable DMA Request */
    CLEAR_BIT(hi2c->Instance->CR2, I2C_CR2_DMAEN);
